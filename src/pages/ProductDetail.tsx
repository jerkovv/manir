...
import { ArrowLeft, ArrowRight, Check, Droplets, ChevronDown, Sparkles, FlaskConical, ShoppingBag, Plus } from "lucide-react";
import { fetchProductBySlug, fetchProducts, productImage, type Product } from "@/lib/products";
...
const AccordionItem = ({ title, icon, children, badge }: AccordionItemProps) => {
  const [isOpen, setIsOpen] = useState(false);
  return (
    <div className="border-b border-border/40 last:border-b-0">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between py-5 group cursor-pointer">
        ...
        <motion.div animate={{ rotate: isOpen ? 180 : 0 }} transition={{ duration: 0.25 }} className="text-muted-foreground/50 group-hover:text-warm-brown transition-colors">
          <ChevronDown size={18} />
        </motion.div>
      </button>
...
const ProductDetail = () => {
  ...
  const allImages = product.images?.length ? product.images : [productImage(product)];
  const hasBenefits = product.benefits?.length > 0;
  const hasFreeFrom = product.free_from?.length > 0;
  const hasUsage = !!product.usage;
  const hasComposition = !!(product.inci || product.composition_note);
  const hasAccordionContent = hasBenefits || hasUsage || hasComposition;
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-24">
            <SectionReveal>
              <div className="lg:sticky lg:top-28">
                <motion.div key={activeImage} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="bg-warm-cream aspect-[4/5] overflow-hidden mb-4">
                  <img src={allImages[activeImage]} alt={product.name} className="w-full h-full object-cover" />
                </motion.div>
                {allImages.length > 1 && (
                  <div className="grid grid-cols-4 gap-2">
                    {allImages.map((img, i) => (
                      <button key={i} onClick={() => setActiveImage(i)} className={`aspect-square overflow-hidden transition-all duration-300 ${activeImage === i ? "ring-2 ring-warm-brown ring-offset-2 ring-offset-background" : "opacity-60 hover:opacity-100"}`}>
            ...
            <SectionReveal delay={0.15}>
              <div>
                <div className="flex items-center gap-3 mb-3">
                  {product.category && (
                    <span className="font-body text-[10px] tracking-[0.25em] uppercase text-muted-foreground">{product.category}</span>
                  )}
                  {product.featured && (
                    <span className="font-body text-[9px] tracking-[0.15em] uppercase bg-warm-brown text-primary-foreground px-2.5 py-1">Izdvojeno</span>
                  )}
                </div>

                <h1 className="font-heading text-4xl md:text-[2.75rem] leading-[1.1] text-foreground mb-3">{product.name}</h1>

                <div className="flex items-center gap-4 mb-6">
                  {product.size && <span className="font-body text-sm text-muted-foreground">{product.size}</span>}
                  {product.active_ingredients_count && (
                    <>
                      <span className="w-px h-4 bg-border" />
                      <span className="font-body text-xs tracking-[0.1em] uppercase text-warm-brown">{product.active_ingredients_count} aktivnih sastojaka</span>
                    </>
                  )}
                </div>

                <div className="mb-8">
                  <p className="font-heading text-3xl text-foreground">{Number(product.price).toLocaleString("sr-RS")} <span className="text-lg text-muted-foreground">RSD</span></p>
                </div>

                <div className="w-12 h-px bg-warm-brown/40 mb-8" />

                {product.short_description && (
                  <p className="font-body text-[15px] text-muted-foreground leading-[1.8] mb-8">{product.short_description}</p>
                )}
                ...
                {product.target_audience && (
                  <div className="flex items-start gap-4 mb-8 p-5 bg-warm-cream/60 border border-border/40">
                    ...
                    <div>
                      ...
                      <p className="font-body text-sm text-muted-foreground leading-relaxed">{product.target_audience}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center gap-4 mb-4">
                  <div className="flex items-center border border-border/60">
                    ...
                    <button onClick={() => setQuantity(q => q + 1)} className="w-11 h-12 flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors"><Plus size={14} /></button>
                  </div>
                  <motion.button onClick={handleAddToCart} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} className="flex-1 inline-flex items-center justify-center gap-3 bg-warm-brown text-primary-foreground px-8 py-4 font-body text-[11px] tracking-[0.2em] uppercase hover:bg-warm-dark transition-colors duration-300">
                ...
                {hasFreeFrom && (
                  <div className="mt-8 flex flex-wrap gap-2">
                    {product.free_from.map((item) => (
                      <span key={item} className="font-body text-[10px] tracking-[0.1em] uppercase text-muted-foreground border border-border/60 px-3 py-1.5">✓ {item}</span>
                    ))}
                  </div>
                )}
                  ...
                  <div className="mt-12 pt-10 border-t border-border/30">
                    ...
                    <div className="bg-[hsl(var(--warm-cream)/0.35)] rounded-sm">
                      <div className="px-4 md:px-7">
                        {hasBenefits && (
                          <AccordionItem title="Benefiti" icon={<Sparkles size={18} strokeWidth={1.5} />} badge={`${product.benefits.length} benefita`}>
                            <div className="space-y-2.5 pl-7">
                              {product.benefits.map((b, i) => (
                                <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }} className="flex items-start gap-2.5">
                        ...
                        {hasUsage && (
                          <AccordionItem title="Način upotrebe" icon={<Droplets size={18} strokeWidth={1.5} />}>
                            <div className="pl-7">
                        ...
                        {hasComposition && (
                          <AccordionItem title="Sastav (INCI)" icon={<FlaskConical size={18} strokeWidth={1.5} />}>
                            <div className="pl-7 space-y-3">
                              {product.composition_note && (
                                <p className="font-body text-[13px] text-muted-foreground leading-relaxed">{product.composition_note}</p>
                              )}
                              {product.inci && (
